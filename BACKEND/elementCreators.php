<?php
    ...
    function CreateBookElement($data, $checked = false):string{
        $result = "
        ...
        if(isset($_SESSION["user_id"])){
            $result .="<label class='ui-like'>
                        <input type='checkbox'".($checked?" checked ":"")." onchange='addOrRemoveFromFavorites(this);' data-isbn-id='$data[ISBN_id]' class='like-input'>
                        <div class='like'>
    ...
    function create_listed_books_elements($title, $genre, $author, $release_date, $lang, $isbn, $page) : string {
        ...
        if(isset($_SESSION["user_id"])){
            $users_favorite_books = get_users_favorite_books($_SESSION["user_id"]);
            $favorite_books_isbn_ids = array_map(function($record){
                return $record["ISBN_id"];
        ...
        for ($i=0; $i < count($data); $i++) { 
            $checked = false;
            if(isset($_SESSION["user_id"]) && in_array($data[$i]["ISBN_id"], $favorite_books_isbn_ids)){
                $checked = true;
            }
            $resultHTML.=CreateBookElement($data[$i], $checked);
        }
        if(count($data)==0){
